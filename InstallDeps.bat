...
@if %errorlevel% equ 0 @(
	...
	@IF EXIST "!DEUSEXFOLDER!\rtx-remix" @(
		@echo There is an existing RTX-REMIX folder, need to temporarily rename it.
		@ren "!DEUSEXFOLDER!\rtx-remix" "rtx-remix_backup"
		@IF ERRORLEVEL 1 @(
	...
	@xcopy /B /s /e "!DEUSEXFOLDER!\rtx-remix_backup\*" "%CD%\DeusEx\deps"
)

@mklink /D "!DEUSEXFOLDER!\rtx-remix" "%CD%\DeusEx\deps" && @(
    @echo.
